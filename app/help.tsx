...
フリー設定
---------------------------------------------
●	＜画面を削除する＞フリー画面をクリアします
●	＜使用回数をリセット＞保存されているボタンの使用頻度の情報をリセットします（表示順の頻度順に影響します）
●	「旧フリーの取込」を押すと旧フリーテキストに蓄積されたデータが取込めます。
●	「フリー」画面の編集で内容の編集が出来ます

a.	タイトル「フリー」を変更すると、再起動後、通常の画面に成ります。（再起動後にフリーは新たに空のものが作られます）
...
e.	「フリーテキストは発声後クリア」 ONでフリーテキスト画面で入力エリアが、発声後自動的にクリアされます。OFFの場合はそのまま残ります。
...
d.  「音量変更」ONにすると、指定の音量で発声されます。＋－で調整できます、その下のスライダーをONにするとスライダーでの調整が可能です
